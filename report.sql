...
FROM DW.FACT_FLIGHTS F JOIN DW.DIM_AIRPORTS A ON F.ORIGIN_AIRPORT_KEY = A.AIRPORT_KEY
GROUP BY A.AIRPORT
ORDER BY AIRPORT;
...
FROM(
SELECT A.AIRLINE,ORIGIN_AIRPORT_KEY,DESTINATION_AIRPORT_KEY
FROM DW.FACT_FLIGHTS F JOIN DW.DIM_AIRLINES A ON F.AIRLINE_KEY = A.AIRLINE_KEY
GROUP BY A.AIRLINE,ORIGIN_AIRPORT_KEY,DESTINATION_AIRPORT_KEY) G
...
SELECT AIRLINE
       ...
       ,DENSE_RANK() OVER(ORDER BY TOTAL_DELAYS DESC) RANKD
FROM(
